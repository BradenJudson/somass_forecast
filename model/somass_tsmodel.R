...
spot <- read.csv("data/Spot and Mean Daily Water Temp Data At Depth 21.09.21.csv") %>% 
...
airtemp <- read.csv("data/alberni_air_temps.csv") %>% 
...
future_temps <- read.csv("data/future_air_temps.csv")
...
for(i in seq(nsim)) {
  ...
  future[,i] <- simulate(opt_arima,
                         ...
                         xreg  = cbind(as.matrix(fourier(STS, K = bf, h = fh)), 
                                       xreg = as.numeric(future_temps[,"temperature"])),
